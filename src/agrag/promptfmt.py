...
from .security.sanitize import datamark, stripDatamarks

_SENT = re.compile(r"(?<=[.!?])\s+|\n+")


def nonceFor(trace_id: str, salt: str = "") -> str:
    return hashlib.blake2b((trace_id + salt).encode(), digest_size=4).hexdigest()


def extractTag(text: str, tag: str) -> str:
...
def buildEvidenceBlock(evidence: Evidence, nonce: str) -> str:
...
_BLOCK = re.compile(r"\[chunk (?P<id>[^\s|\]]+) \| doc (?P<doc>[^\s|\]]+) \| page (?P<page>\d+)[^\]]*\]\n(?P<text>.*?)(?=\n\[chunk |\nEVIDENCE_END)", re.DOTALL)


def parseEvidenceBlocks(prompt: str) -> list[dict]:
    out: list[dict] = []
    for m in _BLOCK.finditer(prompt):
        out.append({"chunk_id": m.group("id"), "doc_id": m.group("doc"), "page": int(m.group("page")), "text": stripDatamarks(m.group("text").strip())})
    return out
