...
async def selfQuery(llm: LLM, query: str, *, timeout_s: float | None = None) -> SelfQuery:
    prompt = f"Split the question into a semantic search string and structured filters over the allowed fields {sorted(_ALLOWED_FIELDS)} using operators {sorted(_ALLOWED_OPS)}.\n<query>{query}</query>"

    try:
        sq, _ = await llm.generateStructured(prompt, SelfQuery, temperature=0.0, timeout_s=timeout_s)
    except Exception:
