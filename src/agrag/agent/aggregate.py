...
class MapReduceAggregator:
    ...
    async def _scan_chunks(self, tenant_id: str) -> tuple[list[Chunk], bool]:
        ...
        chunks = [c for c in await self.deps.docstore.list_chunks(tenant_id) if c.doc_id in ready]
        truncated = len(chunks) > cap
        return chunks[:cap], truncated

    async def aggregate(
        self, step: SubStep, *, tenant_id: str, budget: Budget
    ) -> tuple[Evidence, Computation | None]:
        chunks, truncated = await self._scan_chunks(tenant_id)
        if not chunks:
        ...
        async def extract(chunk: Chunk) -> tuple[Chunk, list[str]]:
            ...
            try:
                res, meta = await self.deps.small_llm.generate_structured(
                    prompt, ExtractedItems, timeout_s=budget.call_timeout_s()
                )
        ...
        for chunk, items in mapped:
            ...
            if contributed:
                contributors.append(
                    ScoredChunk(chunk=chunk, score=1.0, why_relevant="aggregation source")
                )
        ...
        comp = Computation(
            comp_id="agg_" + step.step_id,
            code=f"result = count(unique_items)  # {len(unique)} of {len(chunks)} chunks scanned",
            result=tool.result if tool.ok else len(unique),
            sandbox_run_id=tool.run_id,
        )
        self.deps.tracer.event(
            "aggregate", unique=len(unique), scanned=len(chunks), truncated=truncated
        )
        gaps = (
            ["aggregation scanned a truncated chunk set; count may be a lower bound"]
            if truncated
            else []
        )
