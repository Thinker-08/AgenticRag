...
class MapReduceAggregator:
    ...
    async def _scan_chunks(self, tenant_id: str) -> tuple[list[Chunk], bool]:
        ...
        ready = {d.doc_id for d in await self.deps.docstore.list_docs(tenant_id)
                 if d.status == JobState.READY}
        chunks = [c for c in await self.deps.docstore.list_chunks(tenant_id)
                  if c.doc_id in ready]
    ...
    async def aggregate(self, step: SubStep, *, tenant_id: str, budget: Budget) -> tuple[Evidence, Computation | None]:
        ...
        seen: dict[str, str] = {}                       # normalized -> canonical surface form
        ...
        tool = await asyncio.to_thread(
            self.deps.toolrunner.run, "result = n", inputs, timeout_s=self.deps.settings.sandbox.timeout_s)
        comp = Computation(
