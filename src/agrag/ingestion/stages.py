...
async def makeBlurb(llm: LLM, doc_summary: str, chunk: Chunk, cache: Cache | None) -> str:
    key = f"ctx:{getattr(llm, 'name', 'llm')}:{chunk.content_hash}:{doc_summary[:16]}"

    async def compute() -> str:
        prompt = f"<document_summary>{doc_summary}</document_summary>\n<chunk>{chunk.text[:1200]}</chunk>\nIn 1-2 sentences, situate this chunk within the document (section/entity/period). Output only the context."
        res = await llm.generate(prompt, system=_CTX_SYSTEM, max_tokens=80, temperature=0.0)
        return res.text.strip()[:400]

    if cache is not None:
        return await cache.getOrCompute(key, compute)
...
async def contextualize(chunks: Sequence[Chunk], doc_summary: str, llm: LLM, cache: Cache | None = None) -> list[Chunk]:
    out: list[Chunk] = []
    for c in chunks:
        if not isRetrievable(c) or c.kind == ChunkKind.SUMMARY:
            out.append(c)
            continue

        blurb = await makeBlurb(llm, doc_summary, c, cache)
