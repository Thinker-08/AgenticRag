...
class IngestionService:
    def __init__(self, deps: Deps) -> None:
        self.deps = deps
        self.queue = JobQueue(self._run, concurrency=max(1, deps.settings.agent.slot_concurrency // 2))

    def _oversized(self, data: bytes) -> bool:
        return len(data) > self.deps.settings.parser.max_upload_mb * 1024 * 1024

    async def _quarantine_oversized(self, doc_id: str, tenant_id: str, content_hash: str, filename: str) -> Document:
        doc = self._new_doc(doc_id, tenant_id, content_hash, filename).model_copy(update={
    ...
    async def submit(self, data: bytes, *, tenant_id: str = "default", filename: str = "") -> JobHandle:
        ...
        if existing and existing.status == JobState.READY:
            return JobHandle(doc_id=existing.doc_id, status="ready", deduped=True)   # idempotency (C10)
        if existing and existing.status not in TERMINAL_STATES:
            # same bytes already being ingested: return the in-flight handle, never double-work (C10)
        ...
        await self.deps.docstore.upsert_doc(doc)
        job = Job(job_id=uuid.uuid4().hex[:12], doc_id=doc_id, tenant_id=tenant_id,
                  content_hash=content_hash, trace_id=uuid.uuid4().hex[:12])
        await self.queue.enqueue(job, data)
        return JobHandle(doc_id=doc_id, job_id=job.job_id, status="queued")

    async def ingest(self, data: bytes, *, tenant_id: str = "default", filename: str = "",
    ...
    async def ingest_text(self, text: str, *, tenant_id: str = "default", filename: str = "",
                          doc_id: str | None = None) -> Document:
        return await self.ingest(text.encode("utf-8"), tenant_id=tenant_id,
                                 filename=filename or "text.txt", doc_id=doc_id)
    ...
    def _new_doc(self, doc_id: str, tenant_id: str, content_hash: str, filename: str) -> Document:
        return Document(
            doc_id=doc_id, tenant_id=tenant_id, content_hash=content_hash, filename=filename,
            status=JobState.QUEUED, created_at=_now(),
            embedding_model=self.deps.embedding.model, embedding_version=self.deps.embedding.version,
        )
    ...
    async def _run(self, job: Job, data: bytes) -> None:
        ...
        if doc is None:  # crash-replayed job whose doc record is gone: reconstruct, don't die
            doc = self._new_doc(job.doc_id, job.tenant_id, job.content_hash, "")
            await self.deps.docstore.upsert_doc(doc)
        trace_id = job.trace_id or job.job_id
        with self.deps.tracer.start_trace("ingest", trace_id=trace_id, tenant_id=job.tenant_id, doc_id=job.doc_id):
            try:
                with self.deps.tracer.span("parse"):
                    doc = await self._set(doc, JobState.PARSING)
                    parsed: ParsedDoc = await asyncio.wait_for(
                        self.deps.parser.parse(data, doc_id=job.doc_id, tenant_id=job.tenant_id,
                ...
                with self.deps.tracer.span("embed_index"):
                    ...
                    indexed = await embed_and_index(
                        chunks, embedding=self.deps.embedding, vectorstore=self.deps.vectorstore,
                        lexical=self.deps.lexical, docstore=self.deps.docstore, cache=self.deps.cache)

                await self._set(doc, JobState.READY, indexed_at=_now(),
                ...
                await self._set(doc, JobState.QUARANTINED,
            ...
            except Exception as exc:  # noqa: BLE001 — surface reason, never crash the worker
    ...
    async def _supersede_prior_versions(self, doc: Document) -> None:
        ...
        for prior in await self.deps.docstore.list_docs(doc.tenant_id):
            if (prior.doc_id != doc.doc_id and prior.filename == doc.filename
                    and prior.status == JobState.READY and prior.content_hash != doc.content_hash):
                ...
                await self.deps.docstore.upsert_doc(prior.model_copy(update={"status": JobState.SUPERSEDED}))
