...
async def measureAnnRecall(deps: Deps, *, tenant_id: str = "default", k: int = 10, sample: int = 50) -> dict:
    chunks = await deps.docstore.listChunks(tenant_id)
    if len(chunks) <= k:
        return {"recall_at_k": 1.0, "n_probes": 0, "corpus": len(chunks), "note": "corpus <= k; ANN is exact"}

    texts = [c.embedInput() for c in chunks]
    ids = [c.chunk_id for c in chunks]
    mat = np.asarray(deps.embedding.encodeDocuments(texts).dense, dtype=np.float32)
    ...
    qres = deps.embedding.encodeQueries([texts[pi] for pi in probes])
    hits = 0
