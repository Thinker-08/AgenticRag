...
_STOP = {"the", "a", "an", "of", "to", "in", "on", "and", "or", "is", "are", "was", "were", "be", "for", "with", "as", "at", "by", "from", "that", "this", "it", "its", "s", "was", "has", "had"}
_NEG = {"not", "no", "never", "without", "excluding", "fell", "decreased", "declined"}


def contentWords(text: str) -> list[str]:
...
class LexicalVerifier:
    async def entail(self, premise: str, hypothesis: str, *, budget: Budget | None = None) -> VerdictResult:
        prem_words = set(contentWords(premise))
        hyp_words = contentWords(hypothesis)
        if not hyp_words:
        ...
        hyp_nums = nums(hypothesis)
        prem_nums = nums(premise)
        num_penalty = 0.0
        if hyp_nums:
