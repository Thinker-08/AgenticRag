...
class MemoryVectorStore:
    ...
    def sparseScore(self, q: SparseVector, d: SparseVector) -> float:
        if not q or not d:
            return 0.0
    ...
    async def search(self, query_dense: list[float], *, tenant_id: str, top_k: int = 100, query_sparse: SparseVector | None = None, filters: dict | None = None) -> list[ScoredChunk]:
        ...
        for key, vec in self._dense.items():
            ...
            if rec.chunk.tenant_id != tenant_id:
                continue
            if not matches({**rec.chunk.extra_metadata, "page_no": rec.chunk.page_no, "kind": rec.chunk.kind, "doc_id": rec.chunk.doc_id, "lang": rec.chunk.lang}, filters):
                continue
            dn = float(np.linalg.norm(vec)) or 1.0
            dense = float(np.dot(q, vec) / (qn * dn))
            sparse = self.sparseScore(query_sparse or {}, self._sparse.get(key, {}))
