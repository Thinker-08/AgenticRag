...
def isTable(block_lines: list[str]) -> bool:
...
def isList(block_lines: list[str]) -> bool:
    if len(block_lines) < 2:
        return False
...
class TextParser:
    async def parse(self, data: bytes, *, doc_id: str, tenant_id: str, filename: str = "") -> ParsedDoc:
        ...
        text = clean(data.decode("utf-8", errors="replace"))
        ...
        for pno, raw in enumerate(raw_pages, start=1):
            ...
            for para in re.split(r"\n\s*\n", raw):
                ...
                if isTable(lines):
                    grid = [splitRow(ln) for ln in lines]
                    width = max(len(r) for r in grid)
                    grid = [r + [""] * (width - len(r)) for r in grid]
                    title = breadcrumb[-1] if breadcrumb else ""
                    tbl = Table(block_id=f"{doc_id}:p{pno}:tbl{order}", page=pno, title=title, n_header_rows=1, grid=grid)
                    tbl.validateChecksum()
                    blocks.append(Block(block_id=tbl.block_id, page=pno, type=BlockType.TABLE, text=tbl.linearize(), reading_order=order, table=tbl, breadcrumb=list(breadcrumb)))
                    order += 1
                elif isList(lines):
                    blocks.append(Block(block_id=f"{doc_id}:p{pno}:b{order}", page=pno, type=BlockType.LIST, text="\n".join(ln.strip() for ln in lines), reading_order=order, breadcrumb=list(breadcrumb)))
                    order += 1
                elif len(lines) == 1 and isTitle(lines[0]):
                    breadcrumb = [lines[0].strip()]
                    if not summary:
                        summary = lines[0].strip()
                    blocks.append(Block(block_id=f"{doc_id}:p{pno}:b{order}", page=pno, type=BlockType.TITLE, text=lines[0].strip(), reading_order=order, breadcrumb=list(breadcrumb)))
                    order += 1
                else:
                    body = " ".join(ln.strip() for ln in lines)
                    btype = BlockType.EQUATION if looksLikeEquation(body) else BlockType.PARAGRAPH
                    blocks.append(Block(block_id=f"{doc_id}:p{pno}:b{order}", page=pno, type=btype, text=body, reading_order=order, breadcrumb=list(breadcrumb)))
                    order += 1

            pages.append(Page(page_no=pno, text_ratio=1.0, tier=ParseTier.DIGITAL, blocks=blocks))

        return ParsedDoc(doc_id=doc_id, tenant_id=tenant_id, content_hash="", filename=filename, page_count=len(pages), pages=pages, doc_summary=summary or filename)
