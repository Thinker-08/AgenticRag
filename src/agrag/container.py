...
def _llm(settings: Settings, model: str) -> LLM:
    ...
    if settings.llm.provider == "ollama":
        ...
        return OllamaLLM(model=model, host=settings.llm.host, default_max_tokens=settings.llm.max_tokens)
...
def _vectorstore(settings: Settings):
    ...
    if settings.vectorstore.provider == "qdrant":
        ...
        return QdrantVectorStore(host=settings.vectorstore.host, collection=settings.vectorstore.collection,
                                 dim=settings.embedding.dim, cfg=settings.vectorstore)
...
def build_deps(settings: Settings | None = None) -> Deps:
    ...
    retriever = HybridRetriever(
        embedding=embedding, vectorstore=vectorstore, lexical=lexical,
        reranker=reranker, cfg=settings.retrieval,
    )
    return Deps(
        settings=settings,
        llm=llm,
